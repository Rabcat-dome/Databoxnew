        ...
        <div class="span4">
            <div class="blockoff-right">
                <ul id="person-list" class="nav nav-list">
                    ...
                    <li>
                    <li>
                        <!------ รับค่าส่งกลับมาจาก Controllers   -->
                        <?php
                        $collapse = explode("-", $not2);  // not2 = เท่าค่าที่ส่งรับ ซึ่งข้อมูลที่รับเป็น ไอดี  เช่น 1-2-type_id
                        ...
                        foreach ($data_type_ms as $row) {
                            ...
                            echo "<li>";
                            echo "";
                            echo "<a onClick=select_disvisid('" . $type_id . "-type_id')  href='javascript:;' "
                            . "data-toggle='collapse' data-target='#" . $type_id . "-type_id'>" . $row['type_name'] . "<i class='fa fa-fw fa-caret-down'>"
                            . "</i><i class='icon-chevron-right pull-right'></i></a>";
                            if ($collapse3 != "box") {
                          
                                if ($collapse2 == "type_id") {
                                    if ($type_id == $collapse1) {                                
                                        echo "<ul id='" .$type_id. "-type_id' class=''>";
                                    }
                                    if ($type_id != $collapse1) {
                            ...
                            foreach ($data_group as $row) {
                                ...
                                if ($type_id == $dataId) {
                                    echo "<li>";
                                    echo "<a onClick=select_disvisid('" . $dataId . "-type_id-" . $group_Id . "') >" . $row['groupname'] . "</a>";
                                    echo "</li>";
                                }
                            }
                            echo "</ul>";
            ...
            0 0 0.3em #FF2000;' >" . $this->j3databox->get_num_box_week($ru['group_Id']) . "</div> </h0><br/>" . $short_division_text[0] . "</button>";
                     ...
                     <div class="box" >
                        <div class="box-header">
                            ...
                            <button class="btn btn-box-right" data-toggle="collapse" data-target=""  id="btn-box-right_c_4" style="display: none"  onClick="document.getElementById('box-content_5').style.display = 'none'; document.getElementById('btn-box-right_s_5').style.display = 'block';  document.getElementById('btn-box-right_c_5').style.display = 'none';">
                                    ...
                                    <tr>
                                        <td> <?php
                                            foreach ($unit as $ru) {
                                                ...
                                                if ($ru['group_Id'] == "5" || ($ru['group_Id'] == "3" && $ru['short_division'] == "ผกม.สวฝ.")) {
                                                    $group_Id_r = $ru['group_Id'];
                                                    $disvis_r = $ru['divisid'];
                                                    $short_division = $ru['short_division'];
                                                    $short_division_text = explode('.', $short_division);
            echo "<button   onclick=\"onClick=box_bn('" . $group_Id_r . "-" . $disvis_r . "-box-5')\"  name='Button2' id='button2' value='Post'  class='btn'   style='cursor:pointer' /><h0 style='color:blue'>(" . $this->j3databox->get_num_box($ru['group_Id']) . ") <div style='margin-top: -24px;  margin-left: 60px; width: 0.9%;  height: 5%; text-align:center; position: absolute; color: white;font-size:16px; text-shadow: 0 0 0.3em #FF2000, 0 0 0.3em #FF2000,
            0 0 0.3em #FF2000;' >" . $this->j3databox->get_num_box_week($ru['group_Id']) . "</div> </h0><br/>" . $short_division_text[0] . "</button>";
                                                }
                                            }
                                            ?></td>
